...
var allgemein_text = "<p id='allgemein' style='display: none; font-size:20px;'>Ziel dieses Spieles ist es diese Logikgatter so anzuordnen, damit der Strom (blau) vom unterem Rand des Spielfeldes bis zu den grün und nicht zu den rot hinterlegten Feldern am Kopf gelangt ohne das davor die Zeit abläuft und die Bombe explodiert. <br>Wenn dies geschieht ist das Spiel vorbei und das Level muss von neuem begonnen werden. <br>Während des Rätselns darf der Strom jedoch in die roten Felder fließen. <br><br> Werden Elemente nach ihrer 1. Plazierung noch verschoben, wird dies mit 10 Sekunden Zeitstrafe geahndet.</p>";
...
var text = html + allgemein + link + jsAllgemein + allgemein_button + allgemein_text + logikgatter_text + link + jsAnd + and_button + and_text + link + jsOr + or_button + or_text + 
link + jsXor + xor_button + xor_text + link + jsNot + not + ende + button;
